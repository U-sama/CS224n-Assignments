...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(word_embed_size , hidden_size , bias = True , bidirectional = True ) # e x 1 --> 2h x 1
        
        self.decoder = nn.LSTMCell(word_embed_size + hidden_size , hidden_size , bias = True) # e + h x 1 --> h x 1 
        
        self.h_projection = nn.Linear(hidden_size*2 , hidden_size , bias = False) # 2h x 1 --> h x 1
        
        self.c_projection = nn.Linear(hidden_size*2 , hidden_size , bias = False) # 2h x 1 --> h x 1
        
        self.att_projection = nn.Linear(hidden_size*2 , hidden_size , bias = False) # 2h x 1 --> h x 1 final is 1 
        
        self.combined_output_projection = nn.Linear(hidden_size * 3 , hidden_size , bias = False ) # 3h x1 --> h x 1

        self.target_vocab_projection = nn.Linear(hidden_size , len(self.vocab.tgt) , bias = False) # h x 1 --> vt x 1
        
        self.dropout = nn.Dropout(dropout_rate) # h x 1 --> h x 1
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device).to(device)   # Tensor: (tgt_len, b)
        
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, self.device).to(device) # (s , b, m)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, self.device).to(device) # (s , b, m)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = torch.nn.utils.rnn.pack_padded_sequence(X , source_lengths) # (src_len , b , e) --> (src_len , b , e)
        
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) # (src_len , b , e) --> (src_len , b , 2h) , (2, b, h) ,  (2, b, h)

        enc_hiddens , _ = torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens , batch_first = True) # (src_len , b , 2h)
                
        last_hidden = torch.cat(( last_hidden[0,:,:]  , last_hidden[1,:,:] ) , 1 ) # ( b , 2h)

        init_decoder_hidden = self.h_projection(last_hidden) # (b , 2h) --> (b , h)

        last_cell = torch.cat(( last_cell[0,:,:]  , last_cell[1,:,:] ) , 1 ) # ( b , 2h)

        init_decoder_cell = self.c_projection(last_cell) # (b , 2h) --> (b , h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) #(b, src_len, 2h) --> (b, src_len, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = self.decoder(Ybar_t , dec_state) # (b , e + h) (b , h) --> (dec_hidden, dec_cell) (b , h)
        dec_hidden, dec_cell = dec_state # (b ,h) , (b , h)
        e_t = torch.squeeze( torch.bmm(enc_hiddens_proj , torch.unsqueeze(dec_hidden , 2) ) , 2 ) #(b , src_len , h) x (b , h , 1) --> (b , src_len )
        ...
        a_t = torch.squeeze( torch.bmm(torch.unsqueeze(alpha_t , 1) , enc_hiddens) , 1) # (b , 1 ,src_len) x (b , src_len , 2h) --> (b , 1 , 2h) -->(b , 2h)

        U_t = torch.cat((a_t , dec_hidden ) , 1 ) #( b , h ) cat ( b , 2h) --> (b , 3h)
